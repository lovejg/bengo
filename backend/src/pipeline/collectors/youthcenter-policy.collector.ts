...
export class YouthcenterPolicyCollector implements PolicyCollector {
  ...
  async collect(): Promise<RawPolicyDocument[]> {
    ...
    const keyParam =
      this.configService.get<string>('YOUTHCENTER_POLICY_API_KEY_PARAM') ?? 'apiKeyNm';
    const pageParam = this.configService.get<string>('YOUTHCENTER_POLICY_PAGE_PARAM') ?? 'pageNum';
    const sizeParam =
    ...
    const pageTypeValue = this.configService.get<string>('YOUTHCENTER_POLICY_PAGE_TYPE') ?? '1';
    ...
    const pageSize = Number(this.configService.get<string>('YOUTHCENTER_POLICY_PAGE_SIZE') ?? 100);
    const maxPages = Number(this.configService.get<string>('YOUTHCENTER_POLICY_MAX_PAGES') ?? 30);
    const regionParam =
    ...
    return items.map((item) => {
      const title = pickFirstString(item, ['plcyNm', 'title', '정책명', '서비스명']) ?? '제목 없음';
      ...
      const supportContent = pickFirstString(item, ['plcySprtCn', 'plcyExplnCn', '지원내용']);
      ...
      const regionCodes =
        strictZipRegionCodes.length > 0 ? strictZipRegionCodes : nameBasedRegionCodes;

      const ageInfo = pickFirstString(item, ['ageInfo', '연령정보']);
      const employmentStatus = pickFirstString(item, ['empmSttsCn', '취업상태내용', '취업상태']);
      const educationReq = pickFirstString(item, ['accrRqisCn', '학력요건', '학력']);
      const specializedReq = pickFirstString(item, ['splzRlmRqisCn', '특화분야', '전공요건']);
      const selectionCriteria = pickFirstString(item, ['plcySlcnCn', '선정기준']);
      const additionalQualification = pickFirstString(item, [
        'addAplyQlfcCndCn',
        '추가신청자격조건',
      ]);
      const screeningMethod = pickFirstString(item, ['srngMthdCn', '심사방법']);
      const requiredDocuments = pickFirstString(item, ['sbmsnDcmntCn', '제출서류']);
      const participationTarget = pickFirstString(item, ['ptcpPrpTrgtCn', '참여목적대상']);
      const applicationPeriod = pickFirstString(item, ['aplyYmd', 'rceptBgngYmd']);
      // 선착순 여부 (Y/N)
      const isFirstComeFirstServed = pickFirstString(item, ['sprtArvlSeqYn']) === 'Y';
      // 사업 기간 기타 (예: "예산 소진시까지")
      const bizPeriodEtc = pickFirstString(item, ['bizPrdEtcCn']);
      const minAgeRaw = pickFirstString(item, ['sprtTrgtMinAge', 'minAge', 'ageMinLmt']);
      const maxAgeRaw = pickFirstString(item, ['sprtTrgtMaxAge', 'maxAge', 'ageMaxLmt']);

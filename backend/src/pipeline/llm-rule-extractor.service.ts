...
## 중복 혜택 조건 (conditions에 추가 — 매우 중요)
주의사항(중복혜택 등) 항목에 "중복 신청 불가", "중복 참여 불가", "중복 혜택 불가" 등 특정 프로그램과의 중복 제한이 있으면 **conditionalHints가 아닌 conditions에** 추가하세요.
- key: receiving[프로그램명] 형식의 camelCase (예: receivingNationalEmploymentSupport)
- type: "boolean", op: "=", value: false, verifiable: true
- fact: "answers.receiving[프로그램명]"
- 예시:
  - "국민취업지원제도 수혜자 신청 불가" → { "key": "receivingNationalEmploymentSupport", "label": "국민취업지원제도 수혜 여부", "fact": "answers.receivingNationalEmploymentSupport", "op": "=", "value": false, "message": "국민취업지원제도를 지원받고 있는 경우 중복 신청이 불가능합니다.", "verifiable": true, "type": "boolean" }
  - "서울 청년수당 중복 불가" → { "key": "receivingSeoulYouthAllowance", "label": "서울 청년수당 수혜 여부", "fact": "answers.receivingSeoulYouthAllowance", "op": "=", "value": false, "message": "서울 청년수당을 받고 있는 경우 중복 신청이 불가능합니다.", "verifiable": true, "type": "boolean" }
  - "정부·지자체 유사사업 중복 참여 불가" → { "key": "receivingSimilarGovProgram", "label": "정부·지자체 유사사업 참여 여부", "fact": "answers.receivingSimilarGovProgram", "op": "=", "value": false, "message": "정부·지자체 유사사업 중복 참여자는 신청할 수 없습니다.", "verifiable": true, "type": "boolean" }
...
## summary
정책의 핵심 내용을 **최대 3문장, 200자 이내**로 요약하세요. **"무엇을 지원하는지"(지원 내용)는 반드시 포함**하세요.
- **본문에 명시된 정보만 사용**하세요. 추측이나 일반적 진술로 길이를 채우지 마세요.
- 본문이 짧거나 단순한 정책은 1~2문장으로도 충분합니다. **무리하게 3문장으로 늘리지 마세요.**
- 추상적인 목적 문장("경제적 부담 지원" 등)보다 구체적인 혜택("면접정장 무료 대여", "월세 지원" 등)을 우선합니다.
- 지원 대상은 별도 필드(targetDescription)에 들어가므로 summary에서 굳이 반복하지 않아도 됩니다.
- 좋은 예: "구직 청년에게 면접용 정장을 무료로 대여해주는 서비스. 1인당 연 4회까지 이용 가능하며 정장·셔츠·구두·가방을 함께 빌릴 수 있다."
- 나쁜 예: "취업난으로 어려움을 겪는 청년의 경제적 자립을 돕기 위한 종합적 지원 사업입니다." (지원 내용 불명확, 일반 진술로 채움)
...
- \`"always"\`: **명시적으로 상시/연중 신청·모집**인 경우만 (예: "상시신청", "상시모집", "연중 모집", "연중 접수", "기간 제한 없음")
- \`"always"\`로 분류하지 말아야 할 것:
...
- 본문에 기간 표기가 전혀 없지만 **자격만 충족하면 언제든 신청 가능한 상시 복지 제도/서비스**로 보이면 (예: 기초생활수급, 장애수당, 출산지원금, 의료비 지원 등 "제도성 복지 서비스") \`"always"\`로 판단하세요. 단 모집/공모/한정 사업 뉘앙스가 있으면 적용 금지.
...
const SUMMARY_ONLY_PROMPT = `정책의 핵심 내용을 **최대 3문장, 200자 이내**로 요약하세요.
**"무엇을 지원하는지"(지원 내용)는 반드시 포함**하세요. 본문에 명시된 정보만 사용하고, 추측이나 일반적 진술로 길이를 채우지 마세요. 본문이 짧으면 1~2문장으로도 충분합니다 — 무리하게 늘리지 마세요. 추상적인 목적 문장("경제적 부담 지원" 등)보다 구체적인 혜택("면접정장 무료 대여", "월세 지원" 등)을 우선합니다. 지원 대상은 targetDescription에 별도로 들어가므로 summary에서 반복하지 않아도 됩니다.
...
- "always": 상시 운영/모집. 본문에 기간 표기가 없어도 "자격만 되면 언제든 신청 가능한 상시 복지 제도/서비스"로 보이면 always (단 모집/공모 뉘앙스 있으면 제외)
- 특정 시기 텍스트 (예: "매년 상반기")
- null: 판단 불가
